...
import { buildSwap } from "./buildSwap.js" // or your "prepareSwap" file
...
export async function sendTransaction(inputMint: string, outputMint: string, amount: number) {
  try {
    ...
    //    If you want a real user signature, do NOT sign on the server;
    //    instead return the transaction to the client to sign in their wallet.
    transaction.sign([serverKeypair])
    // If using `customerAccountAddress.payer`, do:
